...
function App() {
  ...
  const addToTeam = (employee) => {
    ...
    newTeam.sort((a, b) => a.age - b.age);
    setTeam(newTeam);
  };

  const removeFromTeam = (employee) => {
    setTeam(team.filter(e => e.id !== employee.id));
  };

  const averageAge = team.reduce((total, member) => total + member.age, 0) / team.length;
    ...
    <div className="App">
      <div className='left'>
        ...
        {employees.map(employee => (
          ...
          key={employee.id} 
          employee={employee} 
          onAdd={() => addToTeam(employee)} 
          isAdded={team.includes(employee)}
          isInTeamList={false}
        />
        ))}
      </div>
      ...
      <div className='right'>
      <div class="align"> <h2>Team</h2>  <span class="sort">SORT BY AGE</span></div>
        {team.sort((a, b) => a.age - b.age).map(member => (
          <EmployeeCard 
          key={member.id} 
          employee={member} 
          onRemove={() => removeFromTeam(member)} 
          isAdded={true}
          isInTeamList={true}
        />
        ))}
        <div className='card'>Average Age: {isNaN(averageAge) ? "N/A" : averageAge.toFixed(2)}</div>
      </div>
    </div>
  );
}
